...
class ImageCodeView(View):
    ...
    def get(self, request):
        ...
        1.接收前端传递过来的uuid
        2.判断uuid是否获取到
        3.通过调用captcha来生成图片验证码（图片二进制）
        4.将图片内容保存到redis中
             uuid作为一个key，内容作为一个value 同时还需要设置一个实效
        5.返回图片二进制
        :param request:
        :return:
        """
        ...
        # 4.将图片内容保存到redis中
        #     uuid作为一个key，内容作为一个value同时还需要设置一个实效
        redis_conn = get_redis_connection('default')
        # key 设置为uuid
        # second 过期秒数 300秒 过期秒数
        # value text
...
class LoginView(View):
    ...
    def post(self,request):
        ...
        # 默认的认证方法是针对于username 字段进行用户名的判断
        # 当前的判断信息是手机号，我们需要修改一下认证字段
